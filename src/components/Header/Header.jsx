import React from "react";
import s from './Header.module.css';
import { NavLink } from "react-router-dom";


const Header = (props) => {
  const avatar = <img className={s.avatar} src={props.state.photo ? props.state.photo : 'data:image/png;base64,'} alt="" />
    return (
      <header className={s.header}>
        <a href="" className={s.homepageLink}>
          Catsbook
        </a>
        <div className={s.loginBlock}>
          {props.state.isAuth? <div className={s.userInfo}>{avatar} {props.state.login} <span onClick={props.logout} className={s.logout}><svg fill="none" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M17 16L21 12M21 12L17 8M21 12L7 12M13 16V17C13 18.6569 11.6569 20 10 20H6C4.34315 20 3 18.6569 3 17V7C3 5.34315 4.34315 4 6 4H10C11.6569 4 13 5.34315 13 7V8" stroke="#d1d5db" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"/></svg></span></div> : <NavLink to={'/login'}>Login</NavLink>}
        </div>
      </header>
    )
}

export default Header;